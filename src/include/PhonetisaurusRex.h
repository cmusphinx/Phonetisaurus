...
struct VectorIntHash {
    size_t operator()(const vector<int>& v) const
    {
        ...
        for (size_t i = 0; i < v.size(); i++)
            result ^= hash_fn(v[i]) + 0x9e3779b9 + (result << 6) + (result >> 2);
        return result;
    }
};

inline bool
operator==(const vector<int>& x, const vector<int>& y)
{
    if (x.size() != y.size())
        return false;
...
int
LoadClusters(const SymbolTable* syms, SymbolMap12M* clusters,
...
template <class Arc>
void
Entry2FSA(const vector<int>& word, VectorFst<Arc>* fsa, size_t maxlen,
          const SymbolMapM21& invmap, bool superfinal = false)
{
    ...
    for (size_t i = 0; i < word.size(); i++) {
        ...
        while (j <= maxlen && i + j <= (size_t)word.size()) {
            ...
            if (invmap_iter != invmap.end())
                fsa->AddArc(i, Arc(invmap_iter->second, invmap_iter->second, Arc::Weight::One(), i + j));
            j++;
        }
        fsa->AddState();
    }
    ...
    if (superfinal) {
        ...
        fsa->AddState();
        fsa->AddArc(word.size() + 1,
                    Arc(1, 1, Arc::Weight::One(), word.size() + 2));
...
public:
    ...
    void Extend(Path* path, const Arc& arc)
    {
        ...
        if (arc.ilabel == 0 && arc.olabel == 0 && arc.weight == Arc::Weight::One())
            return;
...
NShortestPathSpecialized(
    const Fst<RevArc>& ifst, MutableFst<Arc>* ofst,
    const vector<typename Arc::Weight>& distance, size_t beam, size_t nbest,
    PathFilter* path_filter, bool accumulate = false, float delta = kDelta,
    ...
    typename Arc::StateId state_threshold = kNoStateId)
{
    ...
    fst::internal::ShortestPathCompare<StateId, Weight> compare(
        pairs, distance, superfinal, delta);
    vector<StateId> heap;
    ...
    if (ifst.Start() == kNoStateId || distance.size() <= ifst.Start() || distance[ifst.Start()] == Weight::Zero() || less(weight_threshold, Weight::One()) || state_threshold == 0) {
    ...
    while (!heap.empty()) {
        ...
        if (less(limit, Times(d, p.second)) || (state_threshold != kNoStateId && ofst->NumStates() >= state_threshold))
            continue;

        while (r.size() <= p.first + 1)
            r.push_back(0);
        ++r[p.first + 1];
        ...
        if (p.first == superfinal) {
            ...
            while (ofst->Final(tstate) == Weight::Zero()) {
                for (ArcIterator<Fst<Arc>> aiter(*ofst, tstate); !aiter.Done();
                     aiter.Next()) {
            ...
            if (pit == path_filter->path_map.end()) {
                ...
                pit->second.PathWeight = Plus(LogWeight(pit->second.PathWeight),
...
void
ShortestPathSpecialized(
    const Fst<Arc>& ifst, MutableFst<Arc>* ofst,
    vector<typename Arc::Weight>* distance, PathFilter* path_filter,
    size_t beam, const ShortestPathOptions<Arc, Queue, ArcFilter>& opts,
    bool accumulate = false)
{
    ...
    NShortestPathSpecialized(rfst, ofst, *distance, beam, nbest, path_filter,
                             accumulate, opts.delta, opts.weight_threshold,
