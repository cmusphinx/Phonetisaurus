...
public:
    ...
    {
        return (t1.state == t2.state && t1.word == t2.word && t1.hhash == t2.hhash);
        /*
...
template <class H>
VectorFst<StdArc>
WordToRnnLMFst(const vector<string>& word, H& h)
{
    VectorFst<StdArc> fst;
    fst.AddState();
    fst.SetStart(0);
    for (int i = 0; i < word.size(); i++) {
        int hash = h.HashInput(word.begin() + i, word.begin() + i + 1);
        fst.AddState();
...
public:
    ...
    VectorFst<StdArc> WordToRnnLMFst(const vector<string>& word)
    {
        ...
        fst.SetStart(0);
        for (int i = 0; i < word.size(); i++) {
            int hash = d.h.HashInput(word.begin() + i, word.begin() + i + 1);
            fst.AddState();
            fst.AddArc(i, StdArc(hash, hash, StdArc::Weight::One(), i + 1));
        }
    ...
    SimpleResult Decode(const vector<string>& word, int beam, int kMax, int nbest,
                        double thresh, const string& gpdelim,
                        const string& gdelim, const string& skip)
    {
        ...
        for (int i = 0; i < raw_results.size(); i++) {
            ...
            for (vector<Chunk>::const_iterator it = result.begin();
                 it != result.end(); ++it) {
                vector<string> chunk_vec = it->Tokenize<LegacyRnnLMHash>(
                    static_cast<char>(*gpdelim.c_str()),
                    static_cast<char>(*gdelim.c_str()), d.h);
            ...
            while (!sQueue[s].Empty() && k < kMax && n < nbest) {
                ...
                for (ArcIterator<VectorFst<StdArc>> aiter(fst, top->state);
                    ...
                    for (int i = 0; i < map.size(); i++) {
                        Token ntoken(reinterpret_cast<Token*>(top), map[i], arc.nextstate);
