    ...
    double ComputeNet(const T& p, T* t)
    {
        ...
        if (synd.size() > 0) {
            ...
            for (int i = 0; i < order; i++) {
                ...
                for (int j = 1; j <= i; j++)
                    hash[i] += h.primes_[(i * h.primes_[j] + j) % h.primes_.size()] * static_cast<uint64>(t->history[j - 1] + 1);
        ...
        if (t->word != -1) {
            ...
            if (synd.size() > 0) {
                ...
                for (int i = 0; i < order; i++) {
                    ...
                    hash[i] = h.primes_[0] * h.primes_[1] * static_cast<uint64>(h.vocab_[t->word].class_index + 1);
        ...
        return olayer[t->word] * olayer[h.vocab_.size() + h.vocab_[t->word].class_index];
    }
