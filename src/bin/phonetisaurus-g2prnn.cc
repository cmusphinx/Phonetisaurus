...
void
ThreadedEvaluateWordlist(vector<string>& corpus, RMAP& rmap,
                         LegacyRnnLMHash& h, Decodable& s,
                         int FLAGS_threads, int FLAGS_beam, int FLAGS_kmax,
                         int FLAGS_nbest, bool FLAGS_reverse,
...
#pragma omp parallel for
    ...
    for (int x = 0; x < FLAGS_threads; x++) {
        ...
        for (int i = start; i < end; i++) {
            vector<string> graphemes = tokenize_utf8_string(&corpus[i], &FLAGS_gsep);
            if (FLAGS_reverse == true)
                reverse(graphemes.begin(), graphemes.end());

            graphemes.push_back("</s>");
            SimpleResult result = decoder.Decode(graphemes, FLAGS_beam, FLAGS_kmax, FLAGS_nbest,
                                                 FLAGS_thresh, FLAGS_gpdelim, FLAGS_gdelim, FLAGS_skip);
            rmap[i] = result;
        }
    }
...
void
EvaluateWordlist(vector<string>& corpus, LegacyRnnLMHash& h, Decodable& s,
...
void
EvaluateWord(string word, LegacyRnnLMHash& h, Decodable& s, int FLAGS_beam,
             int FLAGS_kmax, int FLAGS_nbest, bool FLAGS_reverse,
             string FLAGS_gpdelim, string FLAGS_gdelim, string FLAGS_skip,
             double FLAGS_thresh, string FLAGS_gsep)
{

    vector<string> graphemes = tokenize_utf8_string(&word, &FLAGS_gsep);
    if (FLAGS_reverse == true)
        reverse(graphemes.begin(), graphemes.end());
    graphemes.push_back("</s>");
...
main(int argc, char* argv[])
{
    ...
    if (FLAGS_rnnlm.compare("") == 0) {
        ...
        if (!rnnlm_ifp.good()) {
            cout << "Faile to open --rnnlm file '" << FLAGS_rnnlm << "'" << endl;
            exit(1);
        }
    }
    ...
    if (FLAGS_wordlist.compare("") != 0) {
        ...
        if (!wordlist_ifp.good()) {
            cout << "Failed to open --wordlist file '" << FLAGS_wordlist << "'"
                 << endl;
            exit(1);
        } else {
            use_wordlist = true;
    ...
    if (use_wordlist == true) {
        if (FLAGS_threads > 1) {
            ThreadedEvaluateWordlist(corpus, rmap, h, s, FLAGS_threads, FLAGS_beam,
                                     FLAGS_kmax, FLAGS_nbest, FLAGS_reverse,
                                     FLAGS_gpdelim, FLAGS_gdelim, FLAGS_skip,
                                     FLAGS_thresh, FLAGS_gsep);
        } else {
            EvaluateWordlist(corpus, h, s, FLAGS_beam, FLAGS_kmax, FLAGS_nbest,
                             FLAGS_reverse, FLAGS_gpdelim, FLAGS_gdelim, FLAGS_skip,
                             FLAGS_thresh, FLAGS_gsep);
        }
    } else {
        EvaluateWord(FLAGS_word, h, s, FLAGS_beam, FLAGS_kmax, FLAGS_nbest,
                     FLAGS_reverse, FLAGS_gpdelim, FLAGS_gdelim, FLAGS_skip,
                     FLAGS_thresh, FLAGS_gsep);
    }
