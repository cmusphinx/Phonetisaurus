...
main(int argc, char** argv)
{
    ...
    if (argc == 1) {
        ...
        printf("\t-class <int>\n");
        printf("\t\tWill use specified amount of classes to decompose vocabulary; "
               "default is 100\n");

        printf("\t-old-classes\n");
        printf("\t\tThis will use old algorithm to compute classes, which results "
               "in slower models but can be a bit more precise\n");

        printf("\t-rnnlm <file>\n");
        printf("\t\tUse <file> to store rnnlm model\n");

        printf("\t-binary\n");
        printf("\t\tRnnlm model will be saved in binary format (default is plain "
               "text)\n");
        ...
        printf("\t-direct <int>\n");
        printf("\t\tSets size of the hash for direct connections with n-gram "
               "features in millions; default is 0\n");

        printf("\t-direct-order <int>\n");
        printf("\t\tSets the n-gram order for direct connections (max %d); default "
               "is 3\n",
               MAX_NGRAM_ORDER);

        printf("\t-bptt <int>\n");
        printf("\t\tSet amount of steps to propagate error back in time; default "
               "is 0 (equal to simple RNN)\n");

        printf("\t-bptt-block <int>\n");
        printf("\t\tSpecifies amount of time steps after which the error is "
               "backpropagated through time in block mode (default 10, update at "
               "each time step = 1)\n");

        printf("\t-one-iter\n");
        printf("\t\tWill cause training to perform exactly one iteration over "
               "training data (useful for adapting final models on different data "
               "etc.)\n");

        printf("\t-anti-kasparek <int>\n");
        printf("\t\tModel will be saved during training after processing specified "
               "amount of words\n");

        printf("\t-min-improvement <float>\n");
        printf("\t\tSet minimal relative entropy improvement for training "
               "convergence; default is 1.003\n");

        printf("\t-gradient-cutoff <float>\n");
        printf("\t\tSet maximal absolute gradient value (to improve training "
               "stability, use lower values; default is 15, to turn off use 0)\n");
        ...
        printf("\t-lm-prob\n");
        printf("\t\tUse other LM probabilities for linear interpolation with rnnlm "
               "model; see examples at the rnnlm webpage\n");

        printf("\t-lambda <float>\n");
        printf("\t\tSet parameter for linear interpolation of rnnlm and other lm; "
               "default weight of rnnlm is 0.75\n");

        printf("\t-dynamic <float>\n");
        printf("\t\tSet learning rate for dynamic model updates during testing "
               "phase; default is 0 (static model)\n");
        ...
        printf("\t-gen <int>\n");
        printf("\t\tGenerate specified amount of words given distribution from "
               "current model\n");

        printf("\t-independent\n");
        printf(
            "\t\tWill erase history at end of each sentence (if used for training, "
            "this switch should be used also for testing & rescoring)\n");
