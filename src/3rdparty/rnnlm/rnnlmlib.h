...
typedef double direct_t; // doubles for ME weights; TODO: check why floats are
                         // not enough for RNNME (convergence problems)
...
enum FileTypeEnum { TEXT,
                    BINARY,
                    COMPRESSED }; // COMPRESSED not yet
                                  // implemented
...
public:
    ...
    direct_t* syn_d; // direct parameters between input and output layer (similar
                     // to Maximum Entropy model parameters)
    ...
    void
    learnVocabFromTrainFile(); // train_file will be used to construct vocabulary
    ...
    // rescoring of nbest lists void restoreWeights2();
    void saveContext();
    void restoreContext();
    void saveContext2();
    ...
    void netFlush();
    void
    netReset(); // will erase just hidden layer state + bptt history + maxent
                // history (called at end of sentences in the independent mode)
